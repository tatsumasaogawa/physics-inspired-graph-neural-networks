# 参考: https://github.com/amazon-science/co-with-gnns-example/blob/main/utils.py
...
class GCN_dev(nn.Module):
    def __init__(self, in_feats, hidden_size, number_classes, dropout, device):
        ...
        self.conv1 = GraphConv(in_feats, number_classes).to(device)
...
def generate_graph(n, d=None, p=None, graph_type='reg', random_seed=0):
    ...
    nx_graph = nx.Graph() # 動かなかったので, OrderedGraph() -> Graph() と変更した
...
def run_gnn_training(q_torch, dgl_graph, net, embed, optimizer, number_epochs, tol, patience, prob_threshold, n, d, seed_value):
    ...
    count = 0       # track number times early stopping is triggesred
